...
class DB:
    def __init__(self, path: str = "winebot.sqlite3") -> None:
        self.path = path
        ...
        *,
        ...
        try:
            ...
            if cur.lastrowid is None:
                raise RuntimeError("Could not create draft")
    ...
    async def get_draft_by_preview(self, chat_id: int, message_id: int) -> dict[str, Any] | None:
        db = await self._connect()
        try:
            cur = await db.execute(
    ...
    async def get_offers(self, draft_id: int) -> list[dict[str, Any]]:
        db = await self._connect()
        try:
            cur = await db.execute(
    ...
    async def update_draft_caption(self, draft_id: int, caption_html: str) -> None:
        db = await self._connect()
        try:
            await db.execute(
                "UPDATE drafts SET caption_html=? WHERE id=?",
                (caption_html, draft_id),
            )
            await db.commit()
        finally:
            await db.close()
